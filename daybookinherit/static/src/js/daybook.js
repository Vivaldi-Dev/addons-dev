odoo.define('dynamic_partner_daybook_inherit.day_bookinherit', function (require) {
    'use strict';
    ...
    var DayBook = AbstractAction.extend({
        ...
        start: function () {
            ...
            var $initialBalanceSection = this.$('.initial_balance');

            if ($initialBalanceSection.length) {

                console.log("Seção .initial_balance encontrada:", $initialBalanceSection);

                $initialBalanceSection.find('#finalbalance').text('Novo valor do saldo inicial');
            } else {

                console.log("Seção .initial_balance não encontrada.");
            }
        ...
        load_data: function (initial_render = true) {
            var self = this;
            console.log("Update Initial Balance");

            // Obter saldo inicial
            this._rpc({
                route: '/day_book/saldo_inicial',
                params: {}
            }).then(data => {
                console.log(data);

                // Renderizar o restante dos dados
                self.$(".categ").empty();
                try {
                    self._rpc({
                        model: 'day_book.day_book',
                        ...
                        if (initial_render) {
                            self.$('.filter_view_db').html(QWeb.render('DayFilterViewinherit', {
                        ...
                        // Renderizar a tabela e atualizar o saldo inicial
                        self.$('.table_view_db').html(QWeb.render('DaytableInherit', {
                       ...
                       var currencyMapping = {
                        '$': 'USD',
                        '€': 'EUR',
                        'MT': 'MZN' // Adicionando o símbolo do metical moçambicano (MZN)
                    }

                    var initialBalance = datas['initial_balance'];
                    var formattedInitialBalance = formatCurrency(initialBalance, datas['currency']);

                    var finalBalance = datas['final_balance'];
                    var formattedFinalBalance = formatCurrency(finalBalance, datas['currency']);

                    self.$('#initial_balance').text('Initial Balance: ' + formattedInitialBalance);
                    self.$('#finalbalancet').text('Final Balance: ' + formattedFinalBalance);

                    function formatCurrency(amount, currency) {
                    var symbol = currency[0];
                    var position = currency[1];
                    var language = currency[2];

                    var currencyCode = currencyMapping[symbol] || 'USD';

                    // Formata o número com duas casas decimais e adiciona separadores de milhares e ponto decimal
        ...
        print_pdf: function (e) {
            ...
            self._rpc({
                ...
                var action = {
                    ...
                    'report_name': 'daybookinherit.day_bookinherit',
                    'report_file': 'daybookinherit.day_bookinherit',
                    'data': {
                        'report_data': data
                    },
                    'context': {
                        'active_model': 'day_book.day_book',
                        'landscape': 1,
                        'daybook_pdf_report': true
                    },
                    'display_name': 'Day Bookinherit',
                };
...
//                return self.do_action(action);
                    ...
                    for (var i = 0; i < data['report_lines'].length; i++) {
                        ...
                        if (account_id == data['report_lines'][i]['id']) {
                            ...
                            $(event.currentTarget).next('tr').find('td ul').after(
                                QWeb.render('SubSectiondbinherit', {
        ...
        show_balance: function (event, initial_render = true) {
            event.preventDefault();
            var self = this;


            var $balance_template = $(QWeb.render('balce'));
            console.log($(QWeb.render('balce')))

            $balance_template.appendTo("#balance_container");
        },
...
//            }
            ...
            // self.$('#finalblace').text('Final Balance: ');
...
//                self.$('#finalblace').text('Final Balance: ' + data['final_balance']);
            });
    ...
    core.action_registry.add("d_d", DayBook);
    return DayBook;
});
